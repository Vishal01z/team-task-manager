  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className={`w-full ${item.light} rounded-full h-2.5`}>
                      <div
                        ...
                        style={{
                          width: `${
                            stats.myTasks.total > 0
                              ? (item.value / stats.myTasks.total) * 100
                              : 0
                          }%`
                        }}
                      />
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto">
              {stats?.overdueTasks?.length > 0 ? (
                stats.overdueTasks.map((task) => (
                  <div key={task._id} className="flex justify-between p-3 bg-red-50 rounded-xl border border-red-100">
                    <div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {stats.tasksPerUser.map((entry) => (
                      <tr key={entry.user._id} className="border-b border-slate-50">
                        <td className="py-3 font-semibold">{entry.user.name}</td>
                        <td className="py-3 text-center">{entry.total}</td>
                        <td className="py-3 text-center">{entry.todo}</td>
                        <td className="py-3 text-center text-blue-600">{entry.inprogress}</td>
                        <td className="py-3 text-center text-green-600">{entry.done}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {/* 🚀 Empty State */}
        {!stats?.totalTasks && (
          <div className="text-center py-16 bg-white rounded-2xl border mt-6">
            <span className="text-6xl">🚀</span>
            <p className="text-slate-500 mt-4 mb-6">No data yet. Create your first project!</p>
